...
def find_best_teachers(student_id):
    ...
    top_matches = matches[:3]  # Limit to top 3 matches
    return [teacher_id for teacher_id, score in top_matches]
...
class TestMatcher(unittest.TestCase):
    ...
    def test_find_best_teachers(self):
        ...
        self.assertGreaterEqual(len(result), 0, f"Expected at least one match, but got {len(result)}.")
        self.assertLessEqual(len(result), 3, f"Expected at most 3 matches, but got {len(result)}.")
